                        ...
                        <div class="card-header align-items-end d-flex justify-content-between">
                           <div>
                            ...
                            <div class="card-header-form">
                                <!-- Search Form -->
                                <form action="{{ route('backend.header-social.index') }}" method="GET" class="d-flex"
                            ...
                            <a href="{{ route('backend.footer-social.create') }}" class="btn btn-primary">Yeni Sosial Şəbəkə yarat</a>
                        </div>
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    @forelse ($socials as $key => $social)
                                        <tr>
                                            <td>{{ $socials->firstItem() + $key }}</td> <!-- Keeps count on pagination -->
                                            <td>
                                            ...
                                            <td class="text-end">
                                                <div class="d-flex justify-content-end " style="gap: 20px">
                                                    <a href="{{ route('backend.footer-social.edit', $social?->id ?? '') }}"
                                                        class="btn btn-warning btn-sm">
                                                        <i class="fas fa-edit"></i> Edit
                                                    </a>

                                                    <form
                                                        action="{{ route('backend.footer-social.destroy', $social?->id ?? '') }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
